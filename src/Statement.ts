...
import fs from "fs"; // ignore ts compiler error, it works fine

interface Transaction {
  Date: string;
  Type: string;
  Amount: number; // this tells typescript what properties my object will have so I don't get compilation errors
  Balance: number;
}
...
class Statement {
  static print(Statement: Transaction[]) {
    ...
    Statement.reverse().forEach((transaction) => {
      ...
      output += `${transaction.Date} || ${credit.padStart(
        7
      )} || ${debit.padStart(7)} || ${transaction.Balance.toFixed(2)}\n`; // eslint forces my code to look like this, im sorry that its hard to read
    });

    fs.writeFile("docs/statement.txt", output, (err: Error) => {
      if (err) throw err;
